...
cu_group_regex = "[{0}]?(?:[{2}]?{3}|[{1}]?[{2}]?)".format(
    cu_hundreds, cu_tens[1:], cu_digits, cu_tens[0]
)
cu_delim_regex = "({0}*{1})".format(cu_thousand, cu_group_regex)
cu_plain_regex = "({0}+[{1}]{2}|(?:{3})$)".format(
    cu_thousand,
...
def swapDigits(input, regex):
    "Swap to digits according to regex provided."

    result = re.sub(
        regex.format(cu_thousand, cu_tens[0], cu_digits), "\g<2>\g<1>", input
    )
    return result
...
class CUNumber:
    ...
    def swapDigits(input):
        """Swap digits in 11-19 unless "і" is "҂"-marked."""

        return swapDigits(input, "(?<!{0})({1})([{2}])")
    ...
    def processGroup(input, group=0):
        ...
        return (
            (cu_dot + cu_thousand * group + CUNumber._processNumberPlain(input % 1000))
            if input % 1000
            else ""
        )
...
class ArabicNumber:
    ...
    def convert(self):
        "Choose CU number conversion mode."
